...
let bf = {
    ...
    //ВАЖНО: если к элементу применён слушатель swipe то все действия по умолчанию у элемента будут убраны (click mousedown touchstart mousemove touchmove mouseup touchend)
    //el - элемент в оболочке ksn библиотеки т.е. $(".exemple")
    ...
    swipe_event: function(el, settings, add_listener = true) {
        console.log("swipe_event_add")
        ...
        let default_settings = {
            ...
            element: el, //передаём объект текущего элемента чтоб с ним можно ыбло взаимодействовать в вункциях обратного вызова callback_*
            min_distance_x: 2, //минимальная дистанция, которую должен пройти указатель, чтобы жест считался как свайп в % от ширины экрана
            max_distance_x: 90, //максимальная дистанция, не превышая которую может пройти указатель, чтобы жест считался как свайп в % от ширины экрана
            min_distance_y: 2, //максимальная дистанция, не превышая которую может пройти указатель, чтобы жест считался как свайп в % от высоты экрана
            max_distance_y: 90, //максимальная дистанция, не превышая которую может пройти указатель, чтобы жест считался как свайп в % от высоты экрана
            ...
            callback_success: () => { //функция которая будет вызвана если жест над элементом был свайпом, тоже самое что и срабатывае события, но выполяняется раньше чем событие убдует инициализированно, а следоовательно запустится раньше чем те функции которые прикреплены к событию swipe
                console.log("SUCCESS")
            },
            callback_fail: () => { //функция которая будет вызвана если жест над элементом не был свайпом
                console.log("FAIL")
            },
            callback_finally: () => { //функция которая будет вызвана по завершенни проверки действия на свайп, вне зависимости как завершилась проверка
                console.log("FINALLY")
            },
            callback_move: () => {
                console.log("move")
        ...
        // настройки по умолчанию объединяем и заменяем настрйоками пользователя
        settings = Object.assign({}, default_settings, settings);
        // настройки по умолчанию объединяем и заменяем настрйоками пользователя

        let start_events = "", //тут будут храниться события которые будут вызываться для начала слушания свайпа mousedown touchstart
            move_events = "", //тут будут храниться события которые будут вызываться во время движения указателя mousemove touchmove
            finall_events = "", //тут будут храниться события которые будут вызываться во время окончания жеста mouseup touchend
            leave_events = "", //тут будут храниться события которые будут вызываться в момент когда указатель покидает элемент или всевозможные ошибки mouseleave touchcancel
            start_time, //время начало свайпа
            total_time, //сколько времени занял свайп
            direction, //направление свайпа
            start_x, //позиция точки нажатия в самом начале по горизонтали
            start_y, //позиция точки нажатия в самом начале по вертикали
            x = 0, //будет хранить изменяемую позицию точки нажатия при перемещении по горизонтали
            y = 0, //будет хранить изменяемую позицию точки нажатия при перемещении по вертикали
            x_dist, //дистанция проейдаеная по горизонтали
            y_dist, //дистанция проейдаеная по вертикали
            min_distance_x, //хранит конвертированное в пиксели знаение минимальной и максимальной дистаций для того чтоб событие считалось свайпом
            max_distance_x, //хранит конвертированное в пиксели знаение минимальной и максимальной дистаций для того чтоб событие считалось свайпом
            min_distance_y, //хранит конвертированное в пиксели знаение минимальной и максимальной дистаций для того чтоб событие считалось свайпом
            max_distance_y; //хранит конвертированное в пиксели знаение минимальной и максимальной дистаций для того чтоб событие считалось свайпом

        if (!settings.touch_swipe && !settings.mouse_swipe) return;

        if (settings.touch_swipe) {
            start_events += start_events.length > 0 ? " touchstart" : "touchstart";
            move_events += move_events.length > 0 ? " touchmove" : "touchmove";
            finall_events += finall_events.length > 0 ? " touchend" : "touchend";
            leave_events += leave_events.length > 0 ? " touchcancel" : "touchcancel";
        }

        if (settings.mouse_swipe) {
            start_events += start_events.length > 0 ? " mousedown" : "mousedown";
            move_events += move_events.length > 0 ? " mousemove" : "mousemove";
            finall_events += finall_events.length > 0 ? " mouseup" : "mouseup";
            leave_events += leave_events.length > 0 ? " mouseleave" : "mouseleave";
        }
        ...
        let start = function(e) {
                ...
                console.log(start_events)
                console.log(move_events)
                console.log(finall_events)
                console.log(leave_events)

                start_time = new Date().getTime(); //время начало свайпа

                //получаем максимально и минимально дупустимые растояния для учёта свайпа
                let scrin_width = $(window).width(),
                    scrin_height = $(window).height();

                min_distance_x = (scrin_width / 100) * settings.min_distance_x;
                max_distance_x = (scrin_width / 100) * settings.max_distance_x;
                min_distance_y = (scrin_width / 100) * settings.min_distance_y;
                max_distance_y = (scrin_width / 100) * settings.max_distance_y;
                //получаем максимально и минимально дупустимые растояния для учёта свайпа

                let event = events_props(e); //получаем объект с событиями для текущего типа устройства
                if (typeof e.touches !== "undefined" && e.touches.length !== 1) return; // игнорирование касания несколькими пальцами

                start_x = event.pageX; //записываем стартовое положение точки нажатия на документе
                start_y = event.pageY; //записываем стартовое положение точки нажатия на документе

                settings.start_x = start_x;
                settings.start_y = start_y;
                settings.left = Number(el.css("left").replace("px", ""));

                x = event.pageX; //записываем текущее положение точки нажатия на документе
                y = event.pageY; //записываем текущее положение точки нажатия на документе

                //добаляем слушатели в зависимости от типов устройств для которых свайп будет отслеживаться
                el.on(move_events, move, { passive: false }); //добавляем слушатель события для перемещения курсора или пальца
                el.on(finall_events, finall, { passive: false }); //добавляем слушатель события когда кнопка мыши отпущена или палец поднят
                el.on(leave_events, leave, { passive: false }); //добавляем слушатель события курсор мыши покинул элемент или палец вышел за пределы экрана или ещё какая-то ошибка на сенсоре
                //добаляем слушатели в зависимости от типов устройств для которых свайп будет отслеживаться
            },
            //стартует после нажатия клавиши мыши или касания пальца на элементе
            ...
            leave = function() {
                settings.allow_leave ? finall() : end(false); //передаём в конечный обработчик результат проверки был ли это свайп
            ...
            clean = function() {
                el.off(move_events, move); //удаляем слушатель события для перемещения курсора или пальца
                el.off(finall_events, finall); //удаляем слушатель события когда кнопка мыши отпущена или палец поднят
                el.off(leave_events, leave); //удаляем слушатель курсор мыши покинул элемент или палец вышел за пределы экрана или ещё какая-то ошибка на 
            },
            //функция чистит все ненужные слушатели

            //оцениваем жест как свайп
            analiz_swipe = function() {
                x_dist = Math.abs(x - start_x); //получаем на сколько пикселей было смещение по горизонтали
                y_dist = Math.abs(y - start_y); //получаем на сколько пикселей было смещение по вертикали

                total_time = (new Date().getTime() - start_time); //сколько времени длился свайп

                //проверяем не превышено ли время разрешённое на свайп
                if (total_time < settings.min_time || total_time > settings.max_time) {
                ...
                if (x_dist >= y_dist) {
                    ...
                    if (x_dist >= min_distance_x && x_dist <= max_distance_x) {
                        if (x > start_x) {
                            direction = "right";
                ...
                //спайп по вертикали
                    ...
                    //если дистанция свайпа больше минимально счиаемой дистанции и меньше максимальной дистации
                    if (y_dist >= min_distance_y && y_dist <= max_distance_y) {
                        if (y > start_y) {
                            direction = "bottom";
            ...
            //запускаем функию end, передав в неё результат проверки действия на свайп, нужно для того чтоб создать прослойку меду функцией приязаной к обработчикам и функцие завершения, чтоб к функции завершения end можно было обратиться и из других мест данной функции
            finall = function() {
                end(analiz_swipe()); //анализируем жест и результат передаём в завершающую функцию
            },
            //запускаем функию end, передав в неё результат проверки действия на свайп

            //завершаем обработку события и в зависимости от того свайп это или нет выполняем определённые действия
            end = function(result) {
                clean(); //функция чистит все ненужные слушатели
                ...
                //сбрасываем некоторые настройки позиции
                settings.x = 0;
                settings.y = 0;
                settings.start_x = 0;
                settings.start_y = 0;
            ...
            run_event = function() {
                ...
                el[0].dispatchEvent(swipeEvent); //привязываем событие к элементу
            };
        //запускаем привязку события


        console.log("click "+start_events+" "+move_events+" "+finall_events+" "+leave_events)
        ...
        if (!add_listener) {
            ...
            console.log("swipe_event_remove");
            el.off("click "+start_events+" "+move_events+" "+finall_events+" "+leave_events, bf.remove_default); //включаем события по умолчанию
            el.off(start_events, start); //отключаем прослушивани событий mousedown touchstart для функции start
            console.log("back_default_events");
            //функция удаляет прослушивание всех событий для обнаружения свайпа
            return; //прерываем дальнейщее выполнение функции
        }
        //если передано что нужно удалить слушатель

        el.on("click "+start_events+" "+move_events+" "+finall_events+" "+leave_events, bf.remove_default, { passive: false }); //отключаем события по умолчанию
        console.log("remove_default_events");
        el.on(start_events, start, { passive: false }); //начинаем слушать событие нажатия мыши или касания
    },
    //функция реализует событие свайпа
...
const KSN_jQuery = {
    ...
    on: function(event, callback, options_event, custom_settings = null, elements = this) {
        ...
        for (let i = 0; i < elements.length; i++) {
            for (let b = 0; b < events.length; b++) {
                ...
                if (ksn_custom_events.hasOwnProperty(events[b])) {
                    ksn_custom_events[events[b]].call("", $(elements[i]), custom_settings, true) //запускаем соответствующую функцию для регистрации данного события
                }
                //если событие кастомное
    ...
    //custom_settings - настрйоки которые будт переданы в кастомные события созданые пользователем такие как swipe
    off: function(event, callback, custom_settings = null, elements = this) {
        let events = bf.string_to_array(event); //преобрзуем строковый список в масив
        ...
        for (let i = 0; i < elements.length; i++) {
            for (let b = 0; b < events.length; b++) {
                ...
                //если событие кастомное
                if (ksn_custom_events.hasOwnProperty(events[b])) {
                    ksn_custom_events[events[b]].call("", $(elements[i]), custom_settings, false) //запускаем соответствующую функцию для регистрации данного события
                }
                //если событие кастомное
